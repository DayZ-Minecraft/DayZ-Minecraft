...
public class GenLayerDayZMinorBiome extends GenLayer {
  ...
  public GenLayerDayZMinorBiome(long l, GenLayer genlayer, IWorldType worldtype) {
    super(l);
    parent = genlayer;
    worldType = worldtype;
  }
    ...
    for (int i = 0; i < par4; i++) {
      for (int j = 0; j < par3; j++) {
        ...
        if (nextInt(3) == 0) {
          ...
          BiomeGenBase biome = worldType.setMinorBiomes(BiomeGenBase.getBiomeGenArray()[k], this);
          if (biome != null) {
            l = biome.biomeID;
          }
